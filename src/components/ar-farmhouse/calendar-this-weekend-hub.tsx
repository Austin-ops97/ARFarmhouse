    ...
    <div className="min-w-0 max-w-full space-y-4 overflow-x-hidden">
      ...
      >
        <div className="relative h-48 sm:h-56">
          <Image src={bundle.hero} alt="" fill className="object-cover" sizes="(max-width: 768px) 100vw, 900px" />
          ...
          <div className="absolute bottom-3 left-3 right-3 flex flex-col gap-3 sm:bottom-4 sm:left-4 sm:right-4 sm:flex-row sm:items-end sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-muted-foreground sm:text-sm">{bundle.dateLabel}</p>
            </div>
        ...
        <div className="grid gap-4 p-5 sm:grid-cols-2">
          <div>
            ...
            <ul className="mt-2 space-y-2">
              {bundle.arrivals.length === 0 ? (
                <li className="ar-nested-well rounded-2xl px-3 py-3 text-sm text-muted-foreground">
                  No arrivals scheduled yet. Add a weekend to the calendar and they will line up here.
                </li>
              ) : (
          ...
          <div>
            ...
            <ul className="mt-2 space-y-2">
              {bundle.departures.length === 0 ? (
                <li className="ar-nested-well rounded-2xl px-3 py-3 text-sm text-muted-foreground">
                  No departures listed yet — soft checkout notes will appear with your first booked stay.
                </li>
              ) : (
        ...
        <div className="border-t border-border/45 px-5 py-4 dark:border-white/10">
          <p className="text-xs text-muted-foreground">{bundle.occupancySummary}</p>
          {bundle.notes && <p className="mt-2 text-sm leading-relaxed text-foreground/90">{bundle.notes}</p>}
        </div>
      </motion.div>
        ...
        <div className={cn(surface, "p-5")}>
          ...
          <ul className="mt-3 space-y-2">
            {bundle.mealPlan.length === 0 ? (
              <li className="ar-nested-well rounded-2xl px-3 py-3 text-sm text-muted-foreground">
                No meals planned yet. When the hub has a real weekend, shared menus show here.
              </li>
            ) : (
        ...
        <div className={cn(surface, "p-5")}>
          ...
          <ul className="mt-3 space-y-2">
            {bundle.activities.length === 0 ? (
              <li className="ar-nested-well rounded-2xl px-3 py-3 text-sm text-muted-foreground">
                No activities on the calendar yet. Post a weekend recap or plan in the feed to build this list.
              </li>
            ) : (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className={cn(surface, "p-5")}>
          ...
          <ul className="mt-3 space-y-2">
            {bundle.grocery.length === 0 ? (
              <li className="ar-nested-well rounded-2xl px-3 py-3 text-sm text-muted-foreground">
                Grocery coordination starts empty. Claim items here once your family begins a shared list.
              </li>
            ) : (
              bundle.grocery.map((g) => {
        ...
        <div className={cn(surface, "p-5")}>
          ...
          <ul className="mt-3 space-y-2">
            {bundle.supplies.length === 0 ? (
              <li className="ar-nested-well rounded-2xl px-3 py-3 text-sm text-muted-foreground">
                No supply notes yet. First-aid kits, spare keys, and dock gear can live here when you add them.
              </li>
            ) : (
          ...
          {bundle.equipment.length === 0 ? (
            <p className="mt-2 text-[13px] text-muted-foreground">No shared equipment notes yet.</p>
          ) : (
            <ul className="mt-2 space-y-2">
      ...
      <div className={cn(surface, "p-5")}>
        ...
        {bundle.packing.length === 0 ? (
          <p className="mt-3 text-sm text-muted-foreground">
            Packing chips appear when someone leaves gentle reminders for the next trip.
          </p>
        ) : (
